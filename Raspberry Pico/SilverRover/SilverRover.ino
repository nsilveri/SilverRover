...
uint16_t rotazione_antenna = STERZO_DRITTO;
...
void cmd_ant(MyCommandParser::Argument *args, char *response) {

  strlcpy(response, "success", MyCommandParser::MAX_RESPONSE_SIZE);
  //Serial.println("Cmd from Keyboard");
  Serial.println("Cmd for antenna");
  uint16_t pos_ant = (int32_t)args[0].asInt64;
  rotazione_antenna = pos_ant;
  pwm.setPWM(13, 0, pos_ant);
}
...
void accelerator_control(uint8_t accelerator_value)
{
  if(accelerator_value != 50)
  {
    ...
    {
      mot_speed_perc = map( motor_speed, 0, 4050, 0, 100);
      motor_speed = map(accelerator_value, 50, 100, 0, 100);
      forward(FORWARD);
    }else if(accelerator_value < 50)
    {
      mot_speed_perc = map( motor_speed, 0, 4050, 0, 100);
      motor_speed = map(accelerator_value, 50, 0, 0, 100);
      forward(BACK);
    }
  }else {
      forward(STOP);
...
void setup() {   //SETUP FOR CORE0
  ...
  delay(2000);
  uint8_t i2cAddr0; 
  uint8_t i2cAddr1 = 0x20;
  uint8_t i2cAddr2 = 0x21;
  uint8_t i2cAddr3 = 0x22;
  uint8_t i2cAddr4 = 0x23;
  uint8_t i2cAddr5 = 0x24;
  uint8_t i2cAddr6 = 0x25;
  uint8_t i2cAddr7 = 0x26;
  uint8_t i2cAddr8 = 0x27;
  /*
  if ((i2cAddr1 = pcf.lookForDevice()) != 0)
    {
        Serial.print("\nPCF8574 Bus Address: ");
        Serial.print(i2cAddr1, HEX);
        i2cAddr0 = i2cAddr1;
    }
    else if ((i2cAddr2 = pcf.lookForDevice()) != 0)
    {
        Serial.print("\nPCF8574 Bus Address: ");
        Serial.print(i2cAddr2, HEX);
        i2cAddr0 = i2cAddr2;
    }
    else if ((i2cAddr3 = pcf.lookForDevice()) != 0)
    {
        Serial.print("\nPCF8574 Bus Address: ");
        Serial.print(i2cAddr3, HEX);
        i2cAddr0 = i2cAddr3;
    }
    else if ((i2cAddr4 = pcf.lookForDevice()) != 0)
    {
        Serial.print("\nPCF8574 Bus Address: ");
        Serial.print(i2cAddr4, HEX);
        i2cAddr0 = i2cAddr4;
    }
    else if ((i2cAddr5 = pcf.lookForDevice()) != 0)
    {
        Serial.print("\nPCF8574 Bus Address: ");
        Serial.print(i2cAddr5, HEX);
        i2cAddr0 = i2cAddr5;
    }
    else if ((i2cAddr6 = pcf.lookForDevice()) != 0)
    {
        Serial.print("\nPCF8574 Bus Address: ");
        Serial.print(i2cAddr6, HEX);
        i2cAddr0 = i2cAddr5;
    }
    else if ((i2cAddr7 = pcf.lookForDevice()) != 0)
    {
        Serial.print("\nPCF8574 Bus Address: ");
        Serial.print(i2cAddr7, HEX);
        i2cAddr0 = i2cAddr7;
    }
    else if ((i2cAddr8 = pcf.lookForDevice()) != 0)
    {
        Serial.print("\nPCF8574 Bus Address: ");
        Serial.print(i2cAddr8, HEX);
        i2cAddr0 = i2cAddr8;
    }
    else
    {
    ...
    pcf.setup(i2cAddr0); // Starts the device with the found I2C address
  //pcf.setup(0x20);
  //pcf.begin();
...
//  pinMode(22, OUTPUT);
  ...
  rotazione_antenna = STERZO_DRITTO;
  pwm.setPWM(13, 0, rotazione_antenna);
  ...
  parser.registerCommand("ANT", "i", &cmd_ant);
...
//    Serial.print("Read ");
//    Serial.println(x, HEX);
    pwm.setPWM(WHEEL_BACKWARD_LEFT, 0, 0);
    pwm.setPWM(WHEEL_FORWARD_RIGHT, 0, 0);
...
//    Serial.println(x, HEX);
    ...
    pwm.setPWM(WHEEL_BACKWARD_LEFT, 0, STOP + motor_speed);
    pwm.setPWM(WHEEL_FORWARD_RIGHT, 0, STOP + motor_speed);
    uint8_t speed_ser = STOP + motor_speed;
    Serial.print("SPEED: ");
...
uint16_t ANT_SX_CORRECTION = 15;
uint16_t ANT_DX_CORRECTION = 85;
uint16_t POST_SX_CORRECTION = 35;
uint16_t POST_DX_CORRECTION = 0;
...
//  int post_sx_ext = map(pulselen - 11, 330, 440, 0, 20);
  if (pulselen > centro)
  {
    pwm.setPWM(0, 0, p_p + ANT_SX_CORRECTION); //-5 p_p
    pwm.setPWM(1, 0, pulselen + ANT_DX_CORRECTION); // pulselen
    //============================================
  ...
  {
    pwm.setPWM(0, 0, pulselen + ANT_SX_CORRECTION); //-5 p_p
    pwm.setPWM(1, 0, p_p + ANT_DX_CORRECTION); //pulselen
    //============================================
    ...
    pwm.setPWM(0, 0, pulselen + ANT_SX_CORRECTION); //-5
    pwm.setPWM(1, 0, pulselen + ANT_DX_CORRECTION);
    ant_sx_speed = 0;
...
void sterzo_posteriore(uint16_t pulselen)
{
  ...
  //    delay(200);
  pwm.setPWM(2, 0, pulselen + POST_SX_CORRECTION); //40
  pwm.setPWM(3, 0, pulselen + POST_DX_CORRECTION);
...
void open_panels()
{
  ...
  {
    pwm.setPWM(15, 0, 299); //posizione ideale antenna durante l'apertura dei pannelli
    pwm.setPWM(13, 0, 610);
    delay(500);
...
void rover_commands(uint8_t str)
{
  ...
  {
    if (rotazione_antenna - 10 <= 610)
    {
      Serial.println(rotazione_antenna);
      rotazione_antenna = rotazione_antenna + 10;
      pwm.setPWM(13, 0, rotazione_antenna);
    }
  } else if (str == 77) //M
  {
    if (rotazione_antenna - 10 >= 100)
    {
      Serial.println(rotazione_antenna);
      rotazione_antenna = rotazione_antenna - 10;
      pwm.setPWM(13, 0, rotazione_antenna);
  ...
  } else if (str == 98) //b
  {
    ...
    */
    ...
    if(motor_speed + 405 <= 4050)//  && motor_speed_id >= 1) 4050
    {
        motor_speed = motor_speed + 405;
        mot_speed_perc = map( motor_speed, 0, 4050, 0, 100);
        //motor_speed_id--;
    ...
    if(motor_speed - 405 >= 0)// && motor_speed_id <= 10)
    {
        motor_speed = motor_speed - 405;
        mot_speed_perc = map( motor_speed, 0, 4050, 0, 100);
        //motor_speed_id++;
